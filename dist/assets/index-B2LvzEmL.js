...
`,ge=`// RGB Split (Chromatic Aberration) Effect Shader
...
`;function xe(c){return`${oe}
${c}`}const be={id:"hue-shift",shader:ue,entryPoint:"hueShiftFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.shift??0,0,0,0])},Se={id:"brightness",shader:ce,entryPoint:"brightnessFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.amount??0,0,0,0])},Pe={id:"contrast",shader:le,entryPoint:"contrastFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.amount??1,0,0,0])},Ge={id:"saturation",shader:de,entryPoint:"saturationFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.amount??1,0,0,0])},we={id:"pixelate",shader:fe,entryPoint:"pixelateFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.size??8,e,t,0])},Be={id:"kaleidoscope",shader:me,entryPoint:"kaleidoscopeFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.segments??6,c.rotation??0,0,0])},Te={id:"mirror",shader:he,entryPoint:"mirrorFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.horizontal?1:0,c.vertical?1:0,0,0])},Ce={id:"rgb-split",shader:ge,entryPoint:"rgbSplitFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.amount??.01,c.angle??0,0,0])},Ue={id:"invert",shader:ve,entryPoint:"invertFragment",uniformSize:0,packUniforms:()=>null},Me={id:"levels",shader:pe,entryPoint:"levelsFragment",uniformSize:32,packUniforms:(c,e,t)=>new Float32Array([c.inputBlack??0,c.inputWhite??1,c.gamma??1,c.outputBlack??0,c.outputWhite??1,0,0,0])},Ae={id:"box-blur",shader:ye,entryPoint:"boxBlurFragment",uniformSize:16,packUniforms:(c,e,t)=>new Float32Array([c.radius??5,e,t,0])},_=new Map([["hue-shift",be],["brightness",Se],["contrast",Pe],["saturation",Ge],["pixelate",we],["kaleidoscope",Be],["mirror",Te],["rgb-split",Ce],["invert",Ue],["levels",Me],["box-blur",Ae]]);function Le(c,e){switch(c){case"hue-shift":return{shift:e.hueShift??0};case"brightness":return{amount:e.brightnessAmt??0};case"contrast":return{amount:e.contrastAmt??1};case"saturation":return{amount:e.saturationAmt??1};case"pixelate":return{size:e.pixelate??8};case"kaleidoscope":return{segments:e.kaleidoscopeSegments??6,rotation:e.kaleidoscopeRotation??0};case"mirror":return{horizontal:!!e.mirrorH,vertical:!!e.mirrorV};case"rgb-split":return{amount:e.rgbSplitAmount??.01,angle:e.rgbSplitAngle??0};case"invert":return{};case"levels":return{inputBlack:e.levelsInputBlack??0,inputWhite:e.levelsInputWhite??1,gamma:e.levelsGamma??1,outputBlack:e.levelsOutputBlack??0,outputWhite:e.levelsOutputWhite??1};case"box-blur":return{radius:e.boxBlurRadius??5}}}function Ie(c,e){return c==="none"?[]:[{id:"demo",type:c,enabled:!0,params:Le(c,e)}]}class C{setOptions(e,t,r,i,n){}}const Re="rgba8unorm";class L extends C{device;pipelines=new Map;bindGroupLayouts=new Map;initialized=!1;frameOptions=null;canvasW=0;canvasH=0;constructor(e){super(),this.device=e,this.createPipelines()}static create(e){return new L(e)}createPipelines(){if(!this.initialized){for(const[e,t]of _)this.createEffectPipeline(t);this.initialized=!0}}createEffectPipeline(e){const t=xe(e.shader),r=this.device.createShaderModule({label:`simplified-effect-${e.id}`,code:t}),i=[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{}}];e.uniformSize>0&&i.push({binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}});const n=this.device.createBindGroupLayout({label:`simplified-effect-${e.id}-layout`,entries:i});this.bindGroupLayouts.set(e.id,n);const a=this.device.createRenderPipeline({label:`simplified-effect-${e.id}-pipeline`,layout:this.device.createPipelineLayout({bindGroupLayouts:[n]}),vertex:{module:r,entryPoint:"vertexMain"},fragment:{module:r,entryPoint:e.entryPoint,targets:[{format:Re}]},primitive:{topology:"triangle-list"}});this.pipelines.set(e.id,a)}setOptions(e,t,r,i,n){this.frameOptions=e,this.canvasW=t,this.canvasH=r}gpuRender(e){const t=this.frameOptions,r=t?Ie(t.effectType,t.effectParams):[];if(r.length===0)return e.inputView;const{finalView:i}=this.applyEffects(r,e);return i}createEffectUniformData(e,t,r){const i=_.get(e.type);return i?i.packUniforms(e.params,t,r):null}applyEffects(e,t){const r=e.filter(l=>l.enabled),i=t.inputView,n=t.outputView;if(r.length===0)return{finalView:i,swapped:!1};let a=i,s=n,u=!1;for(const l of r){const o=this.pipelines.get(l.type),p=this.bindGroupLayouts.get(l.type);if(!o||!p){console.warn(`[EffectsPipeline] No pipeline for effect type: ${l.type}`);continue}const d=this.createEffectUniformData(l,this.canvasW,this.canvasH);this.createGpuRenderPass(t,d,a,p,s,o);const h=a;a=s,s=h,u=!u}return{finalView:a,swapped:u}}createGpuRenderPass(e,t,r,i,n,a){let s=null;t&&(s=this.device.createBuffer({size:t.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.device.queue.writeBuffer(s,0,t.buffer));const u=[{binding:0,resource:e.sampler},{binding:1,resource:r}];s&&u.push({binding:2,resource:{buffer:s}});const l=this.device.createBindGroup({layout:i,entries:u}),o=e.encoder.beginRenderPass({colorAttachments:[{view:n,clearValue:{r:0,g:0,b:0,a:0},loadOp:"clear",storeOp:"store"}]});o.setPipeline(a),o.setBindGroup(0,l),o.draw(6),o.end()}}const Oe=`
...
`;class I extends C{device;bindGroupLayout;outputPipeline;uniformBufferGridOn;uniformBufferGridOff;uniformBufferStackedAlpha;bindGroupCacheGridOn=new Map;bindGroupCacheGridOff=new Map;bindGroupCacheStackedAlpha=new Map;outputWidth=0;outputHeight=0;outputMode="normal";constructor(e,t,r,i,n,a){super(),this.device=e,this.bindGroupLayout=t,this.outputPipeline=r,this.uniformBufferGridOn=i,this.uniformBufferGridOff=n,this.uniformBufferStackedAlpha=a}static create(e){const t=e.createShaderModule({label:"output",code:Oe}),r={size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST},i=e.createBuffer({...r,label:"output-uniform-grid-on"}),n=e.createBuffer({...r,label:"output-uniform-grid-off"}),a=e.createBuffer({...r,label:"output-uniform-stacked-alpha"}),s=e.createBindGroupLayout({label:"output-layout",entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),u=e.createRenderPipeline({label:"output-pipeline",layout:e.createPipelineLayout({bindGroupLayouts:[s]}),vertex:{module:t,entryPoint:"vertexMain"},fragment:{module:t,entryPoint:"fragmentMain",targets:[{format:navigator.gpu.getPreferredCanvasFormat()}]},primitive:{topology:"triangle-list"}});return new I(e,s,u,i,n,a)}updateResolution(e,t){this.outputWidth=e,this.outputHeight=t,this.device.queue.writeBuffer(this.uniformBufferGridOff,0,this.getBufferData(0)),this.device.queue.writeBuffer(this.uniformBufferGridOn,0,this.getBufferData(1)),this.device.queue.writeBuffer(this.uniformBufferStackedAlpha,0,this.getBufferData(2))}getBufferData(e){const t=new ArrayBuffer(16),r=new DataView(t);return r.setUint32(0,e,!0),r.setFloat32(4,this.outputWidth,!0),r.setFloat32(8,this.outputHeight,!0),r.setFloat32(12,0,!0),t}invalidateCache(){this.bindGroupCacheGridOn.clear(),this.bindGroupCacheGridOff.clear(),this.bindGroupCacheStackedAlpha.clear()}createOutputBindGroup(e,t,r){const i=r==="grid"?this.bindGroupCacheGridOn:r==="stackedAlpha"?this.bindGroupCacheStackedAlpha:this.bindGroupCacheGridOff,n=i.get(t);if(n)return n;const a=r==="grid"?this.uniformBufferGridOn:r==="stackedAlpha"?this.uniformBufferStackedAlpha:this.uniformBufferGridOff,s=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:e},{binding:1,resource:t},{binding:2,resource:{buffer:a}}]});return i.set(t,s),s}gpuRender(e){const t=e.inputView,r=this.createOutputBindGroup(e.sampler,t,this.outputMode),i=e.encoder.beginRenderPass({colorAttachments:[{view:e.outputView,clearValue:{r:0,g:0,b:0,a:0},loadOp:"clear",storeOp:"store"}]});return i.setPipeline(this.outputPipeline),i.setBindGroup(0,r),i.draw(6),i.end(),e.outputView}setOptions(e,t,r,i,n){super.setOptions(e,t,r,i,n),this.updateResolution(t,r),this.outputMode=e.outputMode==="grid"?"grid":e.outputMode==="stackedAlpha"?"stackedAlpha":"normal"}}const Ee=`// Copy GPUExternalTexture (VideoFrame / HTMLVideoElement) into a regular rgba8unorm target.
struct VertexOutput {
  @builtin(position) position: vec4f,
  @location(0) uv: vec2f,
};
...
`;class R extends C{device;bindGroupLayout;pipeline;constructor(e,t,r){super(),this.device=e,this.bindGroupLayout=t,this.pipeline=r}static create(e,t="rgba8unorm"){const r=e.createShaderModule({label:"copy-external",code:Ee}),i=e.createBindGroupLayout({label:"copy-external-layout",entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,externalTexture:{}}]}),n=e.createRenderPipeline({label:"copy-external-pipeline",layout:e.createPipelineLayout({bindGroupLayouts:[i]}),vertex:{module:r,entryPoint:"vs_main"},fragment:{module:r,entryPoint:"fs_main",targets:[{format:t}]},primitive:{topology:"triangle-list"}});return new R(e,i,n)}gpuRender(e){const t=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:e.sampler},{binding:1,resource:e.inputView}]}),r=e.encoder.beginRenderPass({colorAttachments:[{view:e.outputView,clearValue:{r:0,g:0,b:0,a:1},loadOp:"clear",storeOp:"store"}]});return r.setPipeline(this.pipeline),r.setBindGroup(0,t),r.draw(6),r.end(),e.outputView}}const Ve=`
struct VertexOutput {
  @builtin(position) position: vec4f,
  @location(0) uv: vec2f,
};
...
`,M=96;class O extends C{device;bindGroupLayout;pipeline;uniformBuffer;uniformBufferData=new ArrayBuffer(M);uniformF32=new Float32Array(this.uniformBufferData);uniformU32=new Uint32Array(this.uniformBufferData);constructor(e,t,r,i){super(),this.device=e,this.bindGroupLayout=t,this.pipeline=r,this.uniformBuffer=i}writeLayerUniforms(e,t,r){const i=this.uniformF32,n=this.uniformU32,a=e.transformParams;i[0]=a.opacity,n[1]=0,i[2]=a.posX,i[3]=a.posY,i[4]=a.scaleX,i[5]=a.scaleY,i[6]=a.rotationZ,i[7]=t,i[8]=r,i[9]=0,n[10]=0,n[11]=0,i[12]=a.rotationX,i[13]=a.rotationY,i[14]=Math.max(a.perspective,.5),i[15]=0,n[16]=0,i[17]=a.posZ,i[18]=0,i[19]=1,i[20]=1,n[21]=0,i[22]=0,i[23]=0,this.device.queue.writeBuffer(this.uniformBuffer,0,this.uniformBufferData)}static create(e,t="rgba8unorm"){const r=e.createShaderModule({label:"video-transform",code:Ve}),i=e.createBuffer({label:"layer-uniform",size:M,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),n=e.createBindGroupLayout({label:"video-transform-layout",entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform",minBindingSize:M}}]}),a=e.createRenderPipeline({label:"video-transform-pipeline",layout:e.createPipelineLayout({bindGroupLayouts:[n]}),vertex:{module:r,entryPoint:"vs_main"},fragment:{module:r,entryPoint:"fs_main",targets:[{format:t}]},primitive:{topology:"triangle-list"}});return new O(e,n,a,i)}gpuRender(e){const t=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:e.sampler},{binding:1,resource:e.inputView},{binding:2,resource:{buffer:this.uniformBuffer}}]}),r=e.encoder.beginRenderPass({colorAttachments:[{view:e.outputView,clearValue:{r:0,g:0,b:0,a:0},loadOp:"clear",storeOp:"store"}]});return r.setPipeline(this.pipeline),r.setBindGroup(0,t),r.draw(6),r.end(),e.outputView}setOptions(e,t,r,i,n){super.setOptions(e,t,r,i,n);const a=i/n,s=t/r;this.writeLayerUniforms(e,a,s)}}class _e{device;texturePing;texturePong;ping;pong;constructor(e,t,r){const i={label:"canvas-ping",size:[t,r],format:"rgba8unorm",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING};this.device=e,this.texturePing=this.device.createTexture(i),this.texturePong=this.device.createTexture({...i,label:"canvas-pong"}),this.ping=this.texturePing.createView(),this.pong=this.texturePong.createView()}destroy(){this.texturePing.destroy(),this.texturePong.destroy()}}function E(c){return c instanceof GPUTexture?c.createView():c}const Fe=`
struct Params {
  outW: u32,
  outH: u32,
  srcW: u32,
...
`,G=512;class Xe{device;computePipeline;renderPipeline;computeBGL;renderBGL;accumR;accumG;accumB;computeParams;renderParams;constructor(e,t){this.device=e,this.init(t)}init(e){const t=this.device,r=G*G*4;this.accumR=t.createBuffer({size:r,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.accumG=t.createBuffer({size:r,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.accumB=t.createBuffer({size:r,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.computeParams=t.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.renderParams=t.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.computeBGL=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:4,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}}]});const i=t.createShaderModule({code:We});this.computePipeline=t.createComputePipeline({layout:t.createPipelineLayout({bindGroupLayouts:[this.computeBGL]}),compute:{module:i,entryPoint:"main"}}),this.renderBGL=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:3,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]});const n=t.createShaderModule({code:Ye});this.renderPipeline=t.createRenderPipeline({layout:t.createPipelineLayout({bindGroupLayouts:[this.renderBGL]}),vertex:{module:n,entryPoint:"vs"},fragment:{module:n,entryPoint:"fs",targets:[{format:e}]}})}render(e,t,r,i){const n=this.device,a=E(e);n.queue.writeBuffer(this.computeParams,0,new Uint32Array([G,r,i,0]));const s=Math.sqrt(i*r/(G*G))*18;n.queue.writeBuffer(this.renderParams,0,new Float32Array([G,s,0,0]));const u=n.createCommandEncoder();u.clearBuffer(this.accumR),u.clearBuffer(this.accumG),u.clearBuffer(this.accumB);const l=n.createBindGroup({layout:this.computeBGL,entries:[{binding:0,resource:a},{binding:1,resource:{buffer:this.accumR}},{binding:2,resource:{buffer:this.accumG}},{binding:3,resource:{buffer:this.accumB}},{binding:4,resource:{buffer:this.computeParams}}]}),o=u.beginComputePass();o.setPipeline(this.computePipeline),o.setBindGroup(0,l),o.dispatchWorkgroups(Math.ceil(r/16),Math.ceil(i/16)),o.end();const p=n.createBindGroup({layout:this.renderBGL,entries:[{binding:0,resource:{buffer:this.accumR}},{binding:1,resource:{buffer:this.accumG}},{binding:2,resource:{buffer:this.accumB}},{binding:3,resource:{buffer:this.renderParams}}]}),d=u.beginRenderPass({colorAttachments:[{view:t.getCurrentTexture().createView(),loadOp:"clear",storeOp:"store",clearValue:{r:.04,g:.04,b:.04,a:1}}]});d.setPipeline(this.renderPipeline),d.setBindGroup(0,p),d.draw(3),d.end(),n.queue.submit([u.finish()])}destroy(){const e=[this.accumR,this.accumG,this.accumB,this.computeParams,this.renderParams];for(const t of e)t?.destroy()}}class Ke{waveform;histogram;vectorscope;constructor(e,t){this.waveform=new De(e,t),this.histogram=new qe(e,t),this.vectorscope=new Xe(e,t)}renderWaveform(e,t,r,i,n=0){this.waveform.render(e,t,r,i,n)}renderHistogram(e,t,r,i,n=0){this.histogram.render(e,t,r,i,n)}renderVectorscope(e,t,r,i){this.vectorscope.render(e,t,r,i)}destroy(){this.waveform.destroy(),this.histogram.destroy(),this.vectorscope.destroy()}}const A="rgba8unorm";class V{device;gpuContext;presentationFormat;sampler;effectsPipeline;outputPipeline;videoTexture=null;videoView=null;videoWidth=0;videoHeight=0;canvasWidth=0;canvasHeight=0;transformPipeline;copyPipeline;pingPongView=null;scopeRenderer;scopeContexts;constructor(e,t,r,i,n,a,s,u,l,o){this.device=e,this.gpuContext=t,this.presentationFormat=r,this.sampler=i,this.effectsPipeline=n,this.outputPipeline=a,this.transformPipeline=u,this.copyPipeline=s,this.scopeRenderer=l,this.scopeContexts=o}static async create(e,t={}){const r=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(!r)throw new Error("No WebGPU adapter");const i=await r.requestDevice(),n=e.getContext("webgpu");if(!n)throw new Error("Could not get WebGPU canvas context");const a=navigator.gpu.getPreferredCanvasFormat(),s=L.create(i),u=I.create(i),l=R.create(i,A),o=O.create(i,A),p=i.createSampler({label:"linear-clamp",magFilter:"linear",minFilter:"linear",addressModeU:"clamp-to-edge",addressModeV:"clamp-to-edge"}),h=!!(t.histogram||t.waveform||t.vectorscope)?new Ke(i,a):null,g=v=>{v&&v.configure({device:i,format:a,alphaMode:"premultiplied"})};return g(t.histogram??null),g(t.waveform??null),g(t.vectorscope??null),new V(i,n,a,p,s,u,l,o,h,t)}configureSurface(){this.gpuContext.configure({device:this.device,format:this.presentationFormat,alphaMode:"premultiplied"})}releaseWorkTextures(){this.videoTexture?.destroy(),this.videoTexture=null,this.videoView=null,this.videoWidth=0,this.videoHeight=0,this.pingPongView?.destroy(),this.pingPongView=null,this.canvasWidth=0,this.canvasHeight=0,this.outputPipeline.invalidateCache()}ensureVideoTexture(e,t){e===this.videoWidth&&t===this.videoHeight&&this.videoTexture&&this.videoView||(this.videoTexture?.destroy(),this.videoWidth=e,this.videoHeight=t,this.videoTexture=this.device.createTexture({label:"video-rgba",size:[e,t],format:A,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING}),this.videoView=this.videoTexture.createView())}ensureCanvasTextures(e,t){e===this.canvasWidth&&t===this.canvasHeight||(this.pingPongView?.destroy(),this.canvasWidth=e,this.canvasHeight=t,this.pingPongView=new _e(this.device,e,t),this.outputPipeline.invalidateCache())}renderVideoFrame(e,t,r,i,n){const a=t.displayWidth,s=t.displayHeight;if(a<1||s<1){t.close(),i();return}const u=e.width,l=e.height;if(u<1||l<1){t.close(),i();return}if(this.configureSurface(),this.ensureVideoTexture(a,s),this.ensureCanvasTextures(u,l),!this.videoView||!this.pingPongView){t.close(),i();return}let o;try{o=this.device.importExternalTexture({source:t})}catch(m){n(String(m)),t.close(),i();return}this.copyPipeline.setOptions(r,u,l,a,s),this.transformPipeline.setOptions(r,u,l,a,s),this.effectsPipeline.setOptions(r,u,l,a,s),this.outputPipeline.setOptions(r,u,l,a,s);const p=this.device.createCommandEncoder({label:"frame"}),d={encoder:p,sampler:this.sampler,inputView:o,outputView:this.videoView};this.copyPipeline.gpuRender({...d}),this.transformPipeline.gpuRender({...d,inputView:this.videoView,outputView:this.pingPongView.ping});const h=this.effectsPipeline.gpuRender({...d,inputView:this.pingPongView.ping,outputView:this.pingPongView.pong});this.outputPipeline.gpuRender({...d,inputView:h,outputView:this.gpuContext.getCurrentTexture().createView()}),this.device.queue.submit([p.finish()]);const g=this.scopeRenderer;if(g){const{histogram:m,waveform:b,vectorscope:f}=this.scopeContexts;m&&g.renderHistogram(h,m,u,l,0),b&&g.renderWaveform(h,b,u,l,0),f&&g.renderVectorscope(h,f,u,l)}const v=this.device.queue.onSubmittedWorkDone?.();v?v.then(()=>{t.close(),i()}):(t.close(),i())}}const N=document.querySelector("#status"),q=document.querySelector("#canvas"),Ze=document.querySelector("#video"),je=document.querySelector("#file"),$e=document.querySelector("#scopeHistogram"),Qe=document.querySelector("#scopeWaveform"),Je=document.querySelector("#scopeVectorscope"),et=$e?.getContext("webgpu")??null,tt=Qe?.getContext("webgpu")??null,rt=Je?.getContext("webgpu")??null,it=document.querySelector("#outputMode"),nt=document.querySelector("#effectType"),at=document.querySelector("#mirrorH"),st=document.querySelector("#mirrorV"),ot=document.querySelector("#paramALabel"),ut=document.querySelector("#paramA"),ct=document.querySelector("#paramAVal"),lt=document.querySelector("#paramBLabel"),dt=document.querySelector("#paramB"),pt=document.querySelector("#paramBVal"),ft=document.querySelector("#paramCLabel"),mt=document.querySelector("#paramC"),ht=document.querySelector("#paramCVal"),gt=document.querySelector("#opacity"),vt=document.querySelector("#opacityVal"),yt=document.querySelector("#posX"),xt=document.querySelector("#posXVal"),bt=document.querySelector("#posY"),St=document.querySelector("#posYVal"),Pt=document.querySelector("#posZ"),Gt=document.querySelector("#posZVal"),wt=document.querySelector("#scaleX"),Bt=document.querySelector("#scaleXVal"),Tt=document.querySelector("#scaleY"),Ct=document.querySelector("#scaleYVal"),Ut=document.querySelector("#rotX"),Mt=document.querySelector("#rotXVal"),At=document.querySelector("#rotY"),Lt=document.querySelector("#rotYVal"),It=document.querySelector("#rotZ"),Rt=document.querySelector("#rotZVal"),Ot=document.querySelector("#perspective"),Et=document.querySelector("#perspectiveVal"),F=document.querySelector("#motionPanel"),W=F?new J(F):void 0,k=document.querySelector("#perfFps"),z=document.querySelector("#perfGpuMs"),D=document.querySelector("#perfPrepMs"),H=document.querySelector("#perfRes"),Vt=k&&z&&D&&H?new se({fps:k,gpuMs:z,prepMs:D,resolution:H}):void 0;if(!navigator.gpu)throw N.textContent="WebGPU is not available here. Try a recent Chromium-based browser with GPU acceleration enabled.",new Error("no webgpu");const Y=await V.create(q,{histogram:et,waveform:tt,vectorscope:rt}),X=new ae(Y.device),K=await X.initialize();K||W?.setError("Optical flow: GPU initialization failed (check the console).");const _t=new Z(Y,{statusEl:N,canvas:q,video:Ze,fileInput:je,opacityInput:gt,opacityVal:vt,posXInput:yt,posXVal:xt,posYInput:bt,posYVal:St,posZInput:Pt,posZVal:Gt,scaleXInput:wt,scaleXVal:Bt,scaleYInput:Tt,scaleYVal:Ct,rotXInput:Ut,rotXVal:Mt,rotYInput:At,rotYVal:Lt,rotZInput:It,rotZVal:Rt,perspectiveInput:Ot,perspectiveVal:Et,outputModeSelect:it,effectSelect:nt,mirrorHInput:at,mirrorVInput:st,paramALabel:ot,paramAInput:ut,paramAVal:ct,paramBLabel:lt,paramBInput:dt,paramBVal:pt,paramCLabel:ft,paramCInput:mt,paramCVal:ht,motionViz:W,perfHud:Vt},K?X:null);_t.start();
